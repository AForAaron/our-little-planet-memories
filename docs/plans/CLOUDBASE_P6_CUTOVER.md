...
1. ~~COS/CAM **长期密钥**写入 Run `S3_*`（替换 STS）~~ **已完成**
2. ~~关闭或收紧 CloudBase PG **外网**~~ **已完成**
3. 确认用量告警；禁止超限自动付费（人工确认）
4. 双方确认改用腾讯默认域名为主入口（产品侧）
...
- 技术侧：`little-planet-web-020` 测试域 live 全链路可用（含同源写、媒体、高德）；现网 Vercel 配置未改、自动部署保持关闭
- 产品侧：主入口切换依赖双方改书签（本文件即通知清单）
