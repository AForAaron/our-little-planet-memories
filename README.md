...
每个回忆详情页都有“装饰这页”入口，支持 36 枚内置贴纸（包含小狗、小柯基、小仓鼠等动物）、自由简笔画、整笔橡皮、拖动、缩放、旋转、层级、透明度和撤销/重做。贴画按标题、媒体、正文、追评、聊天及足迹等内容锚点保存，页面换行或宽度变化后会重新定位；双方的正式数据约 15 秒同步一次，同一元素使用 revision 乐观锁防止静默覆盖。新增贴纸会进入对方的“待你看看”，画笔和后续移动不会重复提醒。

- `demo`：只保留当前页面会话，刷新后重置。
- `live`：需要先应用包含 `entry_canvas_items` 的 `0007` migration，以及加入贴纸提醒关联的 `0008` migration；每帖最多 150 项，每笔最多 2,000 个点且 payload 不超过 64KB。
